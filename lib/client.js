  ...
  factory: (require) => {
    ...
    function SessionTodosPanel({ t: tProp, sessions, pushToComposer }) {
      ...
      react.useEffect(() => {
        ...
        let disposed = false;
        let mq = null;
        // 面板宽度：手机移动端（pointer: coarse，与移动端自动收起同一判定）保持 320px；
        // PC 端稍微加宽到 360px，任务文字有更多呼吸空间。
        const panelWidth = () => {
          let coarse = false;
          try { coarse = window.matchMedia("(pointer: coarse)").matches; } catch { coarse = false; }
          return coarse ? 320 : 360;
        };
        const measure = () => {
        ...
        try {
          ...
          if (mq && typeof mq.addEventListener === "function") mq.addEventListener("change", measure);
        } catch { /* 忽略：不支持 matchMedia 时保持默认宽度 */ }
        return () => {
          disposed = true;
          if (timer !== null) window.clearTimeout(timer);
          if (observer !== null) observer.disconnect();
          window.removeEventListener("resize", measure);
          if (mq && typeof mq.removeEventListener === "function") mq.removeEventListener("change", measure);
        };
      }, [sessionId]);
