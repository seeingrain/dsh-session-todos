...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const s = {
      ...
      // 右侧按钮列：发送/删除/颜色 三按钮纵向排列，与文字同层
      actionsCol: { flex: "none", display: "flex", flexDirection: "column", gap: 4, marginTop: 2 },
      ...
      const panelNode = react.createElement(
        ...
        react.createElement(
          ...
          react.createElement(
            ...
            // 顶部「+」：在列表开头插入新任务（footer「+」= 追加末尾）
            react.createElement(
              "button",
              {
                type: "button",
                style: s.iconBtn,
                title: t("panel.addTopTip"),
                "aria-label": t("panel.addTopTip"),
                "data-dsh-todo-add": "top",
                onClick: (e) => addTask(e, true)
              },
              "+"
            ),
            collapseBtn
