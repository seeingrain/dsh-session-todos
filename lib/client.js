 ...
 *     收缩=只在右上角显示 🎯 图标；展开态右上角有 📌 Pin 开关。
 *     未钉住（Pin 关）时：点面板外任何位置即自动收起——全平台统一（原仅触屏）；
 *     已钉住（Pin 开）时：不自动收起，一直保持展开。
 *  2) 任务行：勾选框 + 文字框 + 右侧按钮两行：第一行 发送 ⏎ / 复制 ⧉，
 *     第二行 颜色 ▦（点开常用色板，选色后任务文本变色）/ 删除 ✕；勾选后置灰、
 *     按钮禁用；勾选/取消立即存；双击原地编辑；单击选中后长按 300ms 拖动排序
 *     （拖拽行实时跟手，手机进入拖动时震动；未选中行按动为原生行为：滚动/选文字）；
 *     列表上下对称的两个「+」：上「+」在开头插入、下「+」追加末尾。
 *     分割线任务（提交文本恰为 ---）：渲染为细横线 + 小 ✕，只能删除，
 *     但可像普通任务一样单击选中、长按拖动调整上下顺序。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 面板钉住（Pin）：全局一份（不分会话，仅前端），"1"=钉住、"0"/无记录=未钉住。
    // 钉住 = 不自动收起；未钉住 = 点面板外即收起（全平台）。
    const PIN_KEY = "dsh-session-todos.pinned";
    function readPinned() {
      try { return window.localStorage.getItem(PIN_KEY) === "1"; } catch { return false; }
    }
    function writePinned(val) {
      try { window.localStorage.setItem(PIN_KEY, val ? "1" : "0"); } catch {}
    }
    ...
    if (
      ...
      tag.textContent = [
        ...
        // 浮动成功提示（toast）：fixed 顶部居中、深色胶囊、淡入淡出，不占布局
        ".dsh-todo-toast{position:fixed;top:18px;left:50%;transform:translateX(-50%);z-index:2147483647;background:rgba(20,20,20,.92);color:#fff;padding:8px 18px;border-radius:999px;font-size:13px;line-height:1.4;white-space:nowrap;max-width:80vw;overflow:hidden;text-overflow:ellipsis;box-shadow:0 4px 18px rgba(0,0,0,.32);pointer-events:none}",
        ".dsh-todo-toast-in{animation:dshTodoToastIn .16s ease-out}",
        ".dsh-todo-toast-out{animation:dshTodoToastOut .28s ease-in forwards}",
        "@keyframes dshTodoToastIn{from{opacity:0;transform:translateX(-50%) translateY(-10px)}to{opacity:1;transform:translateX(-50%) translateY(0)}}",
        "@keyframes dshTodoToastOut{from{opacity:1;transform:translateX(-50%) translateY(0)}to{opacity:0;transform:translateX(-50%) translateY(-8px)}}"
      ].join("\n");
    ...
    const zh = {
      ...
      "panel.pinTip": "钉住面板（不自动收起）",
      "panel.unpinTip": "取消钉住（点面板外自动收起）",
      ...
      "panel.pushed": "任务已发送到文本框",
      "panel.copied": "任务文本已复制",
      "panel.confirmDel": "您确认删除以下任务吗？",
      "panel.doneTip": "已完成",
    ...
    const en = {
      ...
      "panel.pinTip": "Pin panel (keep it open)",
      "panel.unpinTip": "Unpin panel (auto-collapse on outside click)",
      ...
      "panel.pushed": "Task sent to the composer",
      "panel.copied": "Task text copied",
      "panel.confirmDel": "Confirm deleting the following task?",
    ...
    const s = {
      ...
      // Pin 开启态：描边/图标取强调色 + 浅底，一眼看出「已钉住」
      pinOn: {
        borderColor: ACCENT,
        color: ACCENT,
        background: "var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.12))"
      },
      // Pin 关闭态：置灰压暗（与会话行 🎯 徽章「无未完成=灰」的语感一致）
      pinOff: { opacity: 0.6, filter: "grayscale(1)" },
      list: { flex: 1, overflowY: "auto", padding: "8px 8px 4px", display: "flex", flexDirection: "column", gap: 3 },
      // 任务行：单行结构（勾选+文字+右侧按钮两行 [发送/复制 | 颜色/删除]）
      row: { display: "flex", flexDirection: "column", gap: 2, padding: "2px 6px", borderRadius: 8, background: "transparent", border: "1px solid transparent" },
      rowTop: { display: "flex", alignItems: "flex-start", gap: 8 },
      // 右侧按钮区：两行——第一行 发送/复制 并排，第二行 调色盘（左）/删除（右）
      // 无 marginTop：第一行按钮盒(28px)中心与文本首行行盒(20px)中心精确对齐
      actionsCol: { flex: "none", display: "flex", flexDirection: "column", gap: 4, alignItems: "flex-start" },
      ...
      // 分割线任务：细横线（flex:1）+ 低调小 ✕
      dividerLine: { flex: 1, borderTop: "1px solid var(--dsw-alias-border-l2, rgba(0,0,0,.28))" },
      dividerDel: { flex: "none", width: 22, height: 22, padding: 0, border: "none", background: "transparent", color: "var(--dsw-alias-label-secondary, #666)", opacity: 0.5, cursor: "pointer", fontSize: 12, lineHeight: 1, fontFamily: "inherit" },
      checkbox: { flex: "none", width: 16, height: 16, marginTop: 6, accentColor: ACCENT, cursor: "pointer" },
      text: {
    ...
    function TaskRow({
      ...
      onColorMenuToggle, onCopied
    }) {
      // 分割线任务（文本恰为 "---"）：只能删除——无勾选/文字/编辑/功能按钮；
      // 但像普通任务一样可单击选中、长按拖动调整上下顺序
      if (task.divider) {
        const dGesture = selected
        ...
        return react.createElement(
          ...
          react.createElement(
            "div",
            { style: { display: "flex", alignItems: "center", gap: 10 }, "data-dsh-todo-divider": task.id },
          ...
          editing
            ...
            : react.createElement(
                "span",
                {
                  ...
                  onContextMenu: selected ? (e) => e.preventDefault() : undefined
                  // 任务上不再显示 tooltip（title 已移除，保留 aria-label）
                },
    ...
    function SessionTodosPanel({ t: tProp, sessions, pushToComposer }) {
      ...
      const [collapsed, setCollapsed] = react.useState(() => {
        if (readPinned()) return false; // 钉住时恒为展开
        try { return readCollapsed(sessions.list.getSnapshot().current ?? null); } catch { return true; }
      });
      // 钉住（Pin）：全局一份（localStorage，不分会话）。
      // 钉住 = 不自动收起；未钉住 = 点面板外即自动收起（全平台统一）。
      const [pinned, setPinned] = react.useState(() => readPinned());
      const setPinnedPersist = (val) => { setPinned(val); writePinned(val); };
      ...
      // 成功浮动提示（toast）：1.5s 后淡出、1.8s 后移除；重复触发重置计时
      const [toast, setToastState] = react.useState(null); // { msg, leaving }
      const toastTimers = react.useRef([]);
      const setToast = (msg) => {
        toastTimers.current.forEach((x) => window.clearTimeout(x));
        toastTimers.current = [
          window.setTimeout(() => setToastState({ msg, leaving: true }), 1500),
          window.setTimeout(() => { setToastState(null); toastTimers.current = []; }, 1800)
        ];
        setToastState({ msg, leaving: false });
      };
      react.useEffect(() => () => { toastTimers.current.forEach((x) => window.clearTimeout(x)); }, []);
      ...
      // 钉住期间忽略该记录、恒为展开（取消钉住即回到该会话记录的状态）
      react.useEffect(() => {
        if (sessionId === null) return;
        setCollapsed(pinned ? false : readCollapsed(sessionId));
      }, [sessionId, pinned]);
      ...
      react.useEffect(() => {
        ...
        call("get", { sessionId })
          .then((v) => {
            ...
            setTasks(list);
            setUnfinished(sessionId, list.some((x) => !x.done && !x.divider)); // 分割线不算任务
            setNotice("");
          })
            ...
            try {
              ...
              call("get", { sessionId: sid })
                .then((v) => {
                  ...
                  W.loaded[sid] = true;
                  W.tasks[sid] = list;
                  setUnfinished(sid, list.some((x) => !x.done && !x.divider)); // 分割线不算任务
      ...
      const persist = react.useCallback(
        (nextTasks) => {
          ...
          setTasks(nextTasks);
          if (sessionId) {
            setUnfinished(sessionId, nextTasks.some((x) => !x.done && !x.divider)); // 分割线不算任务
      ...
      const changeText = (id, text) => {
        ...
        // 分割线：文本恰为 "---"（Markdown 分割线）→ 特殊任务类型，只能删除
        persist(tasks.map((x) => (x.id === id ? { ...x, text: trimmed, divider: trimmed === "---", color: x.color, done: x.done } : x)));
      };
      const del = (id) => {
        const task = tasks.find((x) => x.id === id);
        // 分割线无内容：直接删除，不走二次确认
        if (!(task && task.divider)) {
          const label = task && task.text && task.text.trim() !== "" ? task.text.trim() : "该待办";
          // 删除前二次确认：标题句 + 空行 + 任务全文
          if (!window.confirm(`${t("panel.confirmDel")}\n\n${label}`)) return;
        }
        if (selectedId === id) setSelectedId(null);
        if (colorMenuId === id) setColorMenuId(null);
        persist(tasks.filter((x) => x.id !== id));
      ...
      const push = (id) => {
        const task = tasks.find((x) => x.id === id);
        if (!task || task.divider || typeof task.text !== "string" || task.text.trim() === "") return;
        const ok = pushToComposer(sessionId, task.text);
        if (!ok) { setNotice(t("panel.saveFail")); return; }
        setToast(t("panel.pushed"));
        // 推送成功：文本自动变绿（已推送标记），覆盖之前的自定义色并落盘
        if (task.color !== PUSHED_GREEN)
          persist(tasks.map((x) => (x.id === id ? { ...x, color: PUSHED_GREEN } : x)));
      };
      ...
      // 点击面板外（消息区）：自动收起——全平台统一（原仅触屏 pointer:coarse 生效）；
      // 钉住（Pin）期间不收起。任何端点色板外都关闭色板。
      ...
      react.useEffect(() => {
        ...
        const onDocClick = (event) => {
          ...
          if (collapsed || pinned || inPanel || inPop) return;
          const now = Date.now();
      ...
      }, [collapsed, sessionId, colorMenuId, pinned]);
      ...
      // 右上角 Pin 开关（取代原「→」收起按钮）：钉住 = 不自动收起；未钉住 = 点面板外收起。
      // 开关态进 localStorage（全局一份），刷新/换端后保持。
      const pinBtn = react.createElement(
        "button",
        {
          type: "button",
          style: { ...s.iconBtn, marginLeft: "auto", ...(pinned ? s.pinOn : s.pinOff) },
          title: pinned ? t("panel.unpinTip") : t("panel.pinTip"),
          "aria-label": pinned ? t("panel.unpinTip") : t("panel.pinTip"),
          "aria-pressed": pinned ? "true" : "false",
          "data-dsh-todo-pin": pinned ? "true" : "false",
          onClick: () => setPinnedPersist(!pinned)
        },
        "📌"
      );

      // 分割线不是任务：进度 n/m 与「未完成」判定都不含它
      const realTasks = tasks.filter((x) => !x.divider);
      const doneCount = realTasks.filter((x) => x.done).length;
      const total = realTasks.length;
      // 展开按钮配色：有未完成任务 = 彩色 🎯；无任务（或全部完成）= 灰色
      // （与会话列表 🎯 徽章的语义一致：只标记「未完成」）。
      const hasUnfinished = tasks.some((x) => !x.done && !x.divider);
      ...
      const panelNode = react.createElement(
        ...
        react.createElement(
          "div",
          {
            ...
            // 点击面板空白处（非任务行/按钮/输入框）→ 取消当前选中
            onClick: (e) => {
              if (suppressClickRef.current) return; // 拖拽/手动滚动收尾的合成 click
              if (Date.now() - swipeLockRef.current < 600) return; // 右滑收起后的合成 click
              const tgt = e.target;
              if (tgt && tgt.closest && tgt.closest("[data-dsh-todo-row], button, input, textarea")) return;
            ...
            tasks.length === 0
              ...
              : tasks.map((task) =>
                  react.createElement(TaskRow, {
                    ...
                    onCopied: () => setToast(t("panel.copied"))
                  })
                )
          ),
          react.createElement(
      ...
      const colorPop = colorMenuId !== null && colorPopPos
        ...
        : null;
      // 成功浮动提示：Fragment 兄弟节点（不受面板 transform 影响），fixed 顶部居中
      const toastNode = toast
        ? react.createElement(
            "div",
            {
              className: toast.leaving ? "dsh-todo-toast dsh-todo-toast-out" : "dsh-todo-toast dsh-todo-toast-in",
              "data-dsh-todo-toast": toast.msg,
              role: "status",
              "aria-live": "polite"
            },
            toast.msg
        ...
        : null;
      return react.createElement(react.Fragment, null, panelNode, expandBtn, colorPop, toastNode);
    }
