...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function TaskRow({
      ...
      // 不可选中、不可拖动；渲染为一条横线 + 右侧小 ✕
      if (task.divider) {
        return react.createElement(
          "div",
          {
            "data-dsh-todo-row": task.id,
            style: { ...s.row, padding: "2px 6px" }
          },
          react.createElement(
            "div",
    ...
    function SessionTodosPanel({ t: tProp, sessions, pushToComposer }) {
      ...
      const onRowPointerDown = (e, id) => {
        ...
        const ptask = tasks.find((x) => x.id === id);
        if (ptask && ptask.divider) return; // 分割线：不可选中、不可拖动
