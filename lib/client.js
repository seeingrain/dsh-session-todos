...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function syncBadges() {
      for (const row of document.querySelectorAll('[role="treeitem"]')) {
        ...
        if (has && existing === null) {
          ...
          // 位置：状态图标（行第一个子节点，纯图标无文本）的右边、标题之前；
          // 无状态图标时（flat 布局）退回标题前。
          const first = row.firstElementChild;
          if (first && (first.textContent || "").trim() === "") {
            first.insertAdjacentElement("afterend", badge);
          } else {
            row.insertBefore(badge, first);
          }
    ...
    const s = {
      ...
      // 任务行外壳：两行结构（上行 勾选+文字+按钮，下行 颜色按钮/调色板）
      ...
      // 下行与文字列对齐（checkbox 16 + gap 8 = 24）
      rowBottom: { display: "flex", alignItems: "center", gap: 6, paddingLeft: 24, flexWrap: "wrap" },
      ...
      // 调色板：常用色圆片 + 默认（内联展开在下行，避免滚动列表里弹层被裁切）
      swatch: { flex: "none", width: 18, height: 18, padding: 0, borderRadius: 9, border: "2px solid rgba(0,0,0,.25)", cursor: "pointer" },
      swatchDefault: { flex: "none", width: 18, height: 18, padding: 0, borderRadius: 9, border: "2px solid var(--dsw-alias-border-l2, rgba(0,0,0,.3))", background: "transparent", cursor: "pointer", display: "inline-flex", alignItems: "center", justifyContent: "center", fontSize: 9, lineHeight: 1, fontFamily: "inherit", color: "var(--dsw-alias-label-secondary, #666)" },
    ...
    function TaskRow({
      ...
      colorMenuOpen, onColorMenuToggle, onColorPick
    }) {
      ...
      return react.createElement(
        ...
        react.createElement(
          "div",
          { style: s.rowBottom, "data-dsh-todo-colorbar": task.id },
          react.createElement(
            "button",
            {
              type: "button",
              ...
              title: "指定文本颜色",
              "aria-label": "指定文本颜色",
              onClick: () => onColorMenuToggle(task.id)
            },
            "🎨"
          ),
          colorMenuOpen
          ...
          colorMenuOpen
            ? react.createElement(
                "button",
                {
                  ...
                  onClick: () => onColorPick(task.id, null)
                },
                "默"
              )
            : null
      ...
      react.useEffect(() => {
        ...
        const onEnd = (e) => {
          ...
          if (shouldClose) {
            ...
            setCollapsed(true);
            writeCollapsed(sessionId, true);
            window.setTimeout(() => { setSwipeAnim(null); el.style.transition = ""; el.style.transform = ""; }, dur + 80);
          } else {
            ...
            tasks.length === 0
              ...
              : tasks.map((task) =>
                  react.createElement(TaskRow, {
                    ...
                    colorMenuOpen: colorMenuId === task.id,
