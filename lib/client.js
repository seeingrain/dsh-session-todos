...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const s = {
      ...
      // 操作成功轻提示（绿色，自动消失）：发送/复制反馈
      toast: { padding: "4px 12px 6px", fontSize: 12, color: "var(--dsw-alias-state-success-primary, #30a46c)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }
    };
    ...
    function SessionTodosPanel({ t: tProp, sessions, pushToComposer }) {
      ...
      // 成功轻提示（toast）：自动消失，重复触发重置计时
      const [toast, setToastState] = react.useState("");
      const toastTimer = react.useRef(null);
      const setToast = (msg, ms = 1800) => {
        setToastState(msg);
        if (toastTimer.current) window.clearTimeout(toastTimer.current);
        toastTimer.current = window.setTimeout(() => { setToastState(""); toastTimer.current = null; }, ms);
      };
      ...
      const panelNode = react.createElement(
        ...
        react.createElement(
          ...
          // 操作成功轻提示（自动消失）
          toast !== "" && react.createElement("div", { style: s.toast, "data-dsh-todo-toast": toast }, toast),
