 ...
 *     按钮禁用；勾选/取消立即存；双击原地编辑；单击选中后拖动排序（未选中
 *     行按动为原生行为：滚动/选文字）；底部「+」新增。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const s = {
      ...
      rowDrag: { background: "var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.09))", border: `1px solid ${ACCENT}`, boxShadow: "0 3px 12px rgba(0,0,0,.22)", position: "relative", zIndex: 2, cursor: "grabbing" },
    ...
    function SessionTodosPanel({ t: tProp, sessions, pushToComposer }) {
      ...
      // 选中态（window 级，重挂载保留）：单击选中/再点取消；只有选中行可拖拽排序
      ...
      // 选中 + 拖动排序（2026-08-29 重构，替代长按 320ms 方案）：
      //  - 单击选中（整行描边）；只有选中行 pointerdown 才「武装」拖拽；
      //  - 武装后位移超 5px 才进入 drag 态并跟随手指重排（阈值内仍是点击）；
      //  - 未选中行按动：不做任何拦截（不 preventDefault、不武装）→ 原生行为
      //    （触摸=滚动，桌面=选文字/拖滚动条），永远不会触发排序逻辑。
      const dragIdRef = react.useRef(null);
      const armRef = react.useRef(null); // { id, x, y }：选中行按下时的指针位置
      ...
      const onRowPointerDown = (e, id) => {
        ...
        if (selectedId !== id) { armRef.current = null; return; } // 未选中：纯原生行为
        armRef.current = { id, x: e.clientX, y: e.clientY };
      };
      ...
      // window 监听接管两个阶段：armed→drag（阈值判定）与 drag→跟随重排+松手持久化。
      ...
      react.useEffect(() => {
        const onMove = (e) => {
          if (drag === null) {
            ...
            if (!arm) return;
            if (Math.abs(e.clientX - arm.x) + Math.abs(e.clientY - arm.y) < 5) return; // 阈值内：仍视为点击
            armRef.current = null;
            dragIdRef.current = arm.id;
            setDrag({ id: arm.id });
            return;
          }
        ...
        const onUp = () => {
          if (drag !== null) {
            ...
            armRef.current = null; // 无位移的点击：交给 onClick 处理选中切换
          }
        };
